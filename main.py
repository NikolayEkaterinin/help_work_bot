        ...
        if message.from_user.id == AUTHORIZED_USER_ID:
            if message.text == '/upload':
                await message.answer(
                    "Пожалуйста, отправьте файл, который вы хотите загрузить.")
                # Установите состояние для обработки загрузки файла
                await WaitForUploadState.upload.set()
                await state.update_data(upload_path=current_path)
                return
        # Проверяем, заблокирован ли пользователь
...
class WaitForUploadState(StatesGroup):
    upload = State()

# Обработайте загрузку файла
@dp.message_handler(content_types=types.ContentType.DOCUMENT, state=WaitForUploadState.upload)
async def handle_upload(message: types.Message, state: FSMContext):
    data = await state.get_data()
    upload_path = data.get('upload_path')
    file = message.document

    # Сохраните загруженный файл в текущем каталоге
    file_path = os.path.join(upload_path, file.file_name)
    await message.document.download(file_path)

    await message.answer(f"Файл '{file.file_name}' успешно загружен в каталог '{upload_path}'")
