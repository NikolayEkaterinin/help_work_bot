...
async def handle_start(message: types.Message):
    ...
    if current_path == base_folder:
        ...
        keyboard.add(types.KeyboardButton('Запрос скрипта АТОЛ'))
...
@dp.message_handler(lambda message: message.text == 'Запрос скрипта АТОЛ')
async def handle_sap_input(message: types.Message, state: FSMContext):
    await message.answer("Добрый день! Вы запустили процесс запроса скрипта\
                         для смены заводского номера на плате АТОЛ")
    await Sap_states.sap.set()
    await state.update_data(prev_message_text=message.text)
